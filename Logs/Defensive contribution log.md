...
- **Forward model** — not yet built; forwards hit so rarely (5.8%) a simple rolling
  rate likely suffices over a full model.
- **Cold-start (GW1–6)** — thin within-season history; needs a position/team
  base-rate prior fallback.
- **Confirmation on more data** — only a partial 2025-26 season exists. Re-validate
  as gameweeks accumulate (which the live append-as-you-go design does naturally).
