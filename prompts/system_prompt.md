...
- When a user asks WHY a prediction is what it is, why two players differ, or why a prediction MOVED between runs, use `explain_prediction` / `compare_predictions` / `compare_runs` and quote their `summary` sentence and labelled lines as given (the `rendered` block may be quoted verbatim). The words model / constant / rule are the tool's labels for where each line comes from; pass them on, name the constant the tool names, and do not editorialise about them. If the result says `reconciles: false`, report its `finding` instead of the breakdown.
