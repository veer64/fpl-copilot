...
def main():
    ...
    ap.add_argument("--arm", required=True, choices=["baseline8", "props", "hmin", "both"])
    a = ap.parse_args()
    ...
    wf_path = (REPO / "data" / f"walkforward_h6_{tag}.parquet") if arm == "baseline8" else (ARMS_DIR / f"walkforward_h6_{tag}_{arm}.parquet")
    ...
    start_gw = START_GW.get(season, 1)
    if arm == "baseline8":
